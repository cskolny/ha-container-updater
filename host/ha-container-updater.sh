#!/bin/bash
# =============================================================================
# ha-container-updater.sh
# Host-side script to update the Home Assistant Docker container.
#
# Called by: ha-container-updater-watcher.service (systemd)
# Never called directly by the HA container.
#
# Usage: ha-container-updater.sh [--compose-dir DIR] [--compose-file FILE]
#                              [--service NAME] [--log-file PATH]
#                              [--prune] [--no-prune]
#
# All arguments are optional; defaults match the HA Container Updater component.
# =============================================================================
...
_log() {
    ...
    local entry="${timestamp} - ${level^^} - (ha-container-updater) - ${message}"
    echo "${entry}"
...
# ── Pre-flight checks ─────────────────────────────────────────────────────────
log_info "━━━ HA Container Updater — starting ━━━"
...
# ── Done ──────────────────────────────────────────────────────────────────────
log_info "━━━ HA Container Updater — finished successfully ━━━"
exit 0