...
5. Fill in the setup form:
   ...
   | Trigger file path | `/tmp/ha-container-updater-trigger` | Must be inside a volume-mounted directory |
...
2. Add `https://github.com/cskolny/ha-container-updater` with category **Integration**
3. Search for "HA Container Updater" and install
...
sudo cp host/ha-container-updater.sh          /usr/local/bin/
sudo cp host/ha-container-updater-watcher.sh  /usr/local/bin/
sudo chmod +x /usr/local/bin/ha-container-updater.sh
sudo chmod +x /usr/local/bin/ha-container-updater-watcher.sh

# 2. Install the systemd unit
sudo cp host/ha-container-updater-watcher.service /etc/systemd/system/

# 3. Edit the service file to match your username and paths
sudo nano /etc/systemd/system/ha-container-updater-watcher.service

# 4. Enable and start
sudo systemctl daemon-reload
sudo systemctl enable ha-container-updater-watcher.service
sudo systemctl start  ha-container-updater-watcher.service
...
```yaml
automation:
  alias: "Auto-update HA Container when update available"
  trigger:
    - platform: state
...
All options are adjustable post-setup via **Settings → Devices & Services → HA Container Updater → Configure**:
...
| `trigger_file_path` | `/tmp/ha-container-updater-trigger` | Path written by HA to signal an update; must be volume-mounted |
...
- The trigger file uses a **magic string** (`ha_container_updater_REQUESTED`) that the watcher validates before acting. Stray or empty files are silently ignored.
...
- Confirm the watcher service is running: `sudo systemctl status ha-container-updater-watcher`
- Confirm `/tmp` (or your custom path) is volume-mounted in `docker-compose.yml`.
- Check the updater log: `tail -f /home/pi/homeassistant/ha-container-updater.log`
